...
export default function LoginPage() {
  ...
  const handleRegister = async (e) => {
    ...
    if (!fullName || !email || !email.includes("@") || !password || !role) {
      setRegisterError("Harap isi nama lengkap, email, kata sandi, dan role terlebih dahulu");
      setRegisterLoading(false);
      return;
    }

    if (password.length < 6) {
      setRegisterError("Kata sandi harus memiliki setidaknya 6 karakter");
      setRegisterLoading(false);
      return;
    }

    try {
      const res = await apiFetch('/api/private/register', {
        method: "POST",
      ...
      setRegisterError(err.message || "Terjadi kesalahan. Silakan coba lagi.");
    } finally {
      setRegisterLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-start justify-center px-4 pt-10 sm:pt-16 md:items-center md:pt-0">
      <div className="w-full max-w-md p-8">
        ...
        <div className="flex bg-gray-100 rounded-xl p-1 mb-6">
          <button
            type="button"
            onClick={() => { setIsRegister(false); setError(""); setRegisterError(""); }}
            className={`flex-1 py-2.5 rounded-lg text-sm font-semibold transition ${
              !isRegister ? "bg-white text-green-700 shadow-sm" : "text-gray-500 hover:text-gray-700"
            }`}
          >
            Login
          </button>
          <button
            type="button"
            onClick={() => { setIsRegister(true); setError(""); setRegisterError(""); }}
            className={`flex-1 py-2.5 rounded-lg text-sm font-semibold transition ${
              isRegister ? "bg-white text-green-700 shadow-sm" : "text-gray-500 hover:text-gray-700"
            }`}
          >
